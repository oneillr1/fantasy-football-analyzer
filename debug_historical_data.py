...
def debug_historical_data():
    ...
    try:
        ...
        current_year = max(analyzer.years)
        ...
        for player_name, position in test_players:
            ...
            cleaned_name = analyzer.clean_player_name(player_name)
            cleaned_for_matching = analyzer.clean_player_name_for_matching(player_name)
            ...
            if current_year in analyzer.advanced_data and position in analyzer.advanced_data[current_year]:
                advanced_df = analyzer.advanced_data[current_year][position]
                ...
                if not player_found:
                    ...
                    if 'Player' in advanced_df.columns:
                        ...
                        for i, name in enumerate(advanced_df['Player'].head(5)):
                            print(f"  {i+1}. '{name}' -> cleaned: '{analyzer.clean_player_name(name)}'")
                    continue
            
            # Check ADP data
            if current_year in analyzer.adp_data:
                adp_df = analyzer.adp_data[current_year]
                ...
                if pos_col:
                    # Filter by position
                    pos_filtered = adp_df[adp_df[pos_col].str.upper() == position.upper()]
            ...
            # Check results data
            if current_year in analyzer.results_data:
                results_df = analyzer.results_data[current_year]
            ...
            hist_perf = analyzer._calculate_historical_performance(player_name, position)
            ...
            if hist_perf.get('years_of_data', 0) > 0:
                print(f"  - ADP differentials: {hist_perf.get('avg_adp_differential', 0):.1f}")
                print(f"  - Fantasy points: {hist_perf.get('avg_fantasy_points', 0):.1f}")
            else:
